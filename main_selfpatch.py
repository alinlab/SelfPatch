...
from selfpatch_vision_transformer import SelfPatchHead, DINOHead
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--arch', default='deit_tiny', type=str,
    ...
    parser.add_argument('--out_dim_selfpatch', default=4096, type=int, help="""Dimensionality of
        the DINO head output. For complex and large datasets large values (like 65k) work well.""")
    ...
    parser.add_argument("--k_num", default=4, type=int, help="top k confident patch")
    ...
    parser.add_argument('--port', type=str, default='29500', help='port number')
    return parser

class MultiCropWrapper(nn.Module):
    def __init__(self, backbone, ca_head, c_head, p_head):
    ...
    def forward(self, x, head_only=False, loc=True):
        ...
        logits = self.ca_head(output, loc)
        return [self.c_head(logits[:,:1]), self.p_head(logits[:,1:])]
...
def train_dino(args):
    ...
    student = MultiCropWrapper(student, 
        SelfPatchHead(embed_dim,num_heads, args.k_num),  
        DINOHead(embed_dim,args.out_dim,use_bn=args.use_bn_in_head,norm_last_layer=args.norm_last_layer),  
        DINOHead(embed_dim,args.out_dim_selfpatch,use_bn=args.use_bn_in_head,norm_last_layer=args.norm_last_layer),  
    )
    teacher = MultiCropWrapper(
        teacher,
        SelfPatchHead(embed_dim,num_heads, args.k_num),
        DINOHead(embed_dim,args.out_dim, args.use_bn_in_head),
        DINOHead(embed_dim, args.out_dim_selfpatch, args.use_bn_in_head),
    )
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 100, header)):
        ...
        # teacher and student forward selfpatches + compute dino loss
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            with torch.no_grad():
                teacher_output = [teacher(torch.cat(images[:2]), head_only=True, loc=True), teacher(torch.cat(images[2:]), head_only=True, loc=True)]  # only the 2 global views selfpatch through the teacher
            student_output = [student(torch.cat(images[:2]), head_only=True, loc=False), student(torch.cat(images[2:]), head_only=True, loc=False)]
            loss, c_loss, p_loss = dino_loss(teacher, student, student_output, teacher_output, epoch, it)
        ...
        metric_logger.update(clsloss=c_loss)
        metric_logger.update(patchloss=p_loss)
...
class DINOLoss(nn.Module):
    ...
    def forward(self, teacher, student, student_output, teacher_output, epoch, it):
        """
